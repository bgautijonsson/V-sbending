...
fasteignamat <- read_excel(
  "2023-12-14/fasteignagjold.xlsx",
  skip = 8,
  col_names = c(
    "svfn",
    "sveitarfelag",
...
d <- read_excel(
  "2023-12-14/Sveitarfélög eftir tegundum eigna.xlsx"
) |>
  janitor::clean_names() |>
  filter(
...
p <- (p1 + p2) / p3 +
  plot_annotation(
    title = "Meðalhækkun fasteignamats og áhrif hennar á fasteignagjöld í sveitarfélögum landsins (2023 - 2024)",
    caption = str_c(
...
ggsave(
  plot = p,
  filename = "2023-12-14/fasteignagjold.png",
